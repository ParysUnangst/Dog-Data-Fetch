...
//Define dog facts array for simplicity
const dogFacts = [
...
app.get('/facts', (req, res) => {
    //Parse the number query parameters
    const numberOfFacts = parseInt(req.query.number);

    //check if a valid number is provided
    if (!isNaN(numberOfFacts) && numberOfFacts > 0) {

        //Return the specified number of facts
        const selectedFacts = dogFacts.slice(0, numberOfFacts);
        res.json({ facts: selectedFacts, success: true });
    }else {
        //if no valid number provided or invalid, return all facts
...
app.get('*', (req, res) => {
    res.status(404).json({ error: 'Invalid endpoint', success: false});
});
